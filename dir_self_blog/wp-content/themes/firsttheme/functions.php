...
// grabs inserted by Jian
function ourws_setup() {

    // This theme uses wp_nav_menu() in one location.
    register_nav_menu('primary', __('Primary Menu', 'ourws'));
}

add_action('after_setup_theme', 'ourws_setup');

if ( ! function_exists( 'ourws_comment' ) ) :
/**
 * Template for comments and pingbacks.
 *
 * To override this walker in a child theme without modifying the comments template
 * simply create your own ourws_comment(), and that function will be used instead.
 *
 * Used as a callback by wp_list_comments() for displaying the comments.
 *
 * @since Our Website 1.0
 */
function ourws_comment( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
		<p><?php _e( '广播:', 'ourws' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( '(编辑)', 'ourws' ), '<span class="edit-link">', '</span>' ); ?></p>
	<?php
			break;
		default :
			...
			<header class="comment-meta comment-author vcard">
				<?php
					...
					printf( '<cite class="fn">%1$s %2$s</cite>',
						...
						( $comment->user_id === $post->post_author ) ? '<span> ' . __( '文章作者', 'ourws' ) . '</span>' : ''
					);
					printf( '<a href="%1$s"><time datetime="%2$s">%3$s</time></a>',
						esc_url( get_comment_link( $comment->comment_ID ) ),
						get_comment_time( 'c' ),
						/* translators: 1: date, 2: time */
						sprintf( __( '%1$s at %2$s', 'ourws' ), get_comment_date(), get_comment_time() )
					);
				?>
			</header><!-- .comment-meta -->

			<?php if ( '0' == $comment->comment_approved ) : ?>
				<p class="comment-awaiting-moderation"><?php _e( '您的评论正等待审核。', 'ourws' ); ?></p>
			<?php endif; ?>

			<section class="comment-content comment">
				<?php comment_text(); ?>
				<?php edit_comment_link( __( '编辑', 'ourws' ), '<p class="edit-link">', '</p>' ); ?>
			</section><!-- .comment-content -->

			<div class="reply">
				<?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( '回复', 'ourws' ), 'after' => ' <span>&darr;</span>', 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
			</div><!-- .reply -->
		</article><!-- #comment-## -->
	<?php
		break;
...
if ( ! function_exists( 'ourws_add_face_images' ) ) :
function ourws_add_face_images(){
$content = 
'<div id="smileylink">'.
'<a onclick="javascript:grin(&quot;:?:&quot;)"><img src="'. get_bloginfo('template_url'). '/smilies/icon_question.gif" title="汗" alt="汗" /></a>'.
'<a onclick="javascript:grin(&quot;:razz:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_razz.gif" title="色" alt="色" /></a>'.
'<a onclick="javascript:grin(&quot;:sad:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_sad.gif" title="悲" alt="悲" /></a>'.
'<a onclick="javascript:grin(&quot;:evil:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_evil.gif" title="闭嘴" alt="闭嘴" /></a>'.
'<a onclick="javascript:grin(&quot;:!:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_exclaim.gif" title="调皮" alt="调皮" /></a>'.
'<a onclick="javascript:grin(&quot;:smile:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_smile.gif" title="笑" alt="笑" /></a>'.
'<a onclick="javascript:grin(&quot;:oops:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_redface.gif" title="惊" alt="惊" /></a>'.
'<a onclick="javascript:grin(&quot;:grin:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_biggrin.gif" title="亲" alt="亲" /></a>'.
'<a onclick="javascript:grin(&quot;:eek:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_surprised.gif" title="雷" alt="雷" /></a>'.
'<a onclick="javascript:grin(&quot;:shock:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_eek.gif" title="馋" alt="馋" /></a>'.
'<a onclick="javascript:grin(&quot;:???:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_confused.gif" title="晕" alt="晕" /></a>'.
'<a onclick="javascript:grin(&quot;:cool:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_cool.gif" title="酷" alt="酷" /></a>'.
'<a onclick="javascript:grin(&quot;:lol:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_lol.gif" title="奸" alt="奸" /></a>'.
'<a onclick="javascript:grin(&quot;:mad:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_mad.gif" title="怒" alt="怒" /></a>'.
'<a onclick="javascript:grin(&quot;:twisted:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_twisted.gif" title="狂" alt="狂" /></a>'.
'<a onclick="javascript:grin(&quot;:roll:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_rolleyes.gif" title="萌" alt="萌" /></a>'.
'<a onclick="javascript:grin(&quot;:wink:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_wink.gif" title="吃" alt="吃" /></a>'.
'<a onclick="javascript:grin(&quot;:idea:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_idea.gif" title="贪" alt="贪" /></a>'.
'<a onclick="javascript:grin(&quot;:arrow:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_arrow.gif" title="囧" alt="囧" /></a>'.
'<a onclick="javascript:grin(&quot;:neutral:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_neutral.gif" title="羞" alt="羞" /></a>'.
'<a onclick="javascript:grin(&quot;:cry:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_cry.gif" title="哭" alt="哭" /></a>'.
'<a onclick="javascript:grin(&quot;:mrgreen:&quot;)"><img src="'.get_bloginfo('template_url').'/smilies/icon_mrgreen.gif" title="嘿" alt="嘿" /></a>'.
